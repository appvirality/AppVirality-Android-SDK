...
public class CampaignHandler {
	...
	public static void showLaunchBar(final Activity activity,
		...
		if (campaignDetails != null) {
					...
					try {
						if (activity != null && !activity.isFinishing()) {
							if (campaignDetails.EnableLaunchBar) {
								new AVLaunchBar(activity)
								.showLaunchBanner(campaignDetails);
							} else {
								Log.d("AppviralitySDK",
										"you have disabled launch bar");
							}
						}
					} catch (Exception e) {
					}
			...
			Log.d("AppviralitySDK",
					"you don't have any active campaigns at this time.");
		}
	}
					...
					try {
						if (activity != null && !activity.isFinishing()) {
							if (campaignDetails.EnablePopup) {
								new AVLaunchBar(activity)
								.showLaunchPopup(campaignDetails);
							} else {
								Log.d("AppviralitySDK",
										"you have disabled launch popup");
							}
						}
					} catch (Exception e) {
					}
	...
	public static void showGrowthHack(final Activity activity,
		...
		if (campaignDetails != null) {
			showGrowthHack(activity, campaignDetails.growthHackType,
					campaignDetails);
		}
	}

	public static void showGrowthHack(final Activity activity,
			final AppviralityAPI.GH growthType, CampaignDetails campaignDetails) {
		if (campaignDetails != null) {
			try {
				setCampaignDetails(campaignDetails);
				Intent growthhack = new Intent(activity,
						com.appvirality.wom.ShowGrowthHack.class);
				...
				Log.e("AppviralitySDK",
						"please add ShowGrowthHack activity in your manifest");
			} catch (Exception e) {
			...
			AppviralityAPI.setGrowthhackHandler(activity, growthType,
				...
				public void onCampaignReady(
					...
					if (campaignDetails != null) {
						CampaignHandler.showGrowthHack(activity,
								growthType, campaignDetails);
					} else {
